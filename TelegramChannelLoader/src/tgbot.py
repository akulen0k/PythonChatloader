...
class TgBot:
    ...
    def get_messages_by_date(self, channel_id, date: datetime):  # sync!!!
        ...
        for msg in self.client.iter_messages(channel_id, offset_date=tomorrow, wait_time=5):
    ...
    async def get_all_messages(self, channel_id, channel_name, start_date=None, end_date=None, check_db=True):
        ...
        while not compare_dates(start_date, end_date):
            ch = await self.find_channel_raw(channel_id)
            messages = self.get_messages_by_date(ch, start_date)
    ...
    async def find_channel_raw(self, channle_id):
        res = await self.client.get_entity(PeerChannel(channle_id))
